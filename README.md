...
# Frontend
cd ipl-auction-tracker
npm run lint    # ESLint across all frontend source files
```

> **Note:** No automated test files have been written yet. The `node --test` infrastructure is configured but returns no results. Manual testing has been performed across all workflows. Automated tests are the highest priority for the next development phase.
...
1. Provision a Node.js server (any VPS or PaaS — Render recommended)
...
2. Serve `dist/` from a static host (Vercel, Netlify, Nginx, CDN)
3. Ensure the host has SPA routing configured (all paths serve `index.html`)

### CORS Configuration

In production, update the CORS configuration in `ipl-auction-tracker-backend/src/index.js` to specify the exact frontend origin:
...
- Sport tournaments can only be created after the parent festival's roster is finalised
- If using auction mode, complete the festival auction first
- If using manual mode, lock the team assignments first
...
| `team_owner` | Access festivals and sport tournaments where assigned as an owner or captain. Place bids in live auctions. |
